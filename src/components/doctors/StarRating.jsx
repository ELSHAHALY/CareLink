...
export function calculateRatings(ratings = [], appointments = []) {
  ...
  const validRatings = ratings.filter((r) =>
    eligiblePatients.has(r.patientId),
  )
...
export default function StarRating({ ratings = [], appointments = [] }) {
  ...
  return (
    <div className="star-rating" role="img" aria-label={`Rating: ${average.toFixed(1)} out of 5 stars based on ${count} reviews`}>
      <div className="star-rating__stars">
        {[0, 1, 2, 3, 4].map(renderStar)}
      </div>
      <span className="star-rating__value">{average.toFixed(1)}</span>
      <span className="star-rating__count">({count} review{count !== 1 ? 's' : ''})</span>
    </div>
  )
}
